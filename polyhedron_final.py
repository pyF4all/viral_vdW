...
# Clase centrada en el tratamiento y cálculo de propiedades de poliedros (algunas de las funciones solo son aplicables a 
...
class Polyhedron:
    ...
    @classmethod
    def icosahedron(cls, edge_length=1, *, fold=None, rotation=None, pretrans=None, postrans=None, trans_extra=None, rot_extra=None):
        ...
        if trans_extra is not None:
            verts = cls._transform(verts, np.array([0,0,0]), np.array([0,0,0]), np.array(trans_extra))
        if rot_extra is not None:
            verts = cls._transform(verts, np.array(rot_extra), np.array([0,0,0]), np.array([0,0,0]))
    ...
    # Función que da la coordenada z del punto más bajo del sistema
    def z_min(self):

        z_min = min(i for v in self._z_ranges.values() for i in v)

        return z_min
    
    def z_max(self):

        z_max = max(i for v in self._z_ranges.values() for i in v)

        return z_max

    # Función que devuelve los centros de las caras como una lista de listas
    def face_center(self):

        centers = []

        for face in self.faces:
            center_x = (self.points[face[0]].x + self.points[face[1]].x + self.points[face[2]].x)/3
            center_y = (self.points[face[0]].y + self.points[face[1]].y + self.points[face[2]].y)/3
            center_z = (self.points[face[0]].z + self.points[face[1]].z + self.points[face[2]].z)/3 
            centers.append([center_x, center_y, center_z])

        return centers
    
    # Función que devuelve las coordenadas de los vértices de cada cara
    def face_vertices(self):
        ...
        for face in self.faces:
            ...
            punto1 = [self.points[face[0]].x, self.points[face[0]].y, self.points[face[0]].z]
            punto2 = [self.points[face[1]].x, self.points[face[1]].y, self.points[face[1]].z]
            punto3 = [self.points[face[2]].x, self.points[face[2]].y, self.points[face[2]].z]
            
            vertices_face.append([punto1, punto2, punto3])
    ...
    # Calcula el punto central del icosahedro
    def center(self):
        
        x_s = np.sum(np.array([self.points[p].x for p in self.points]))/20
        y_s = np.sum(np.array([self.points[p].y for p in self.points]))/20
        z_s = np.sum(np.array([self.points[p].z for p in self.points]))/20
    
        p0 = np.array([x_s, y_s, z_s])
    ...
    def area_ico(self, z_i, a):
        ...
        for face in self.faces: 
            ...
            # En el resto de casos se calcula el polígono que se encuentra bajo z_i, y se suma su área
                ...
                poligon = pn.Polygon(np.array([p1,p21,p31]))
                area += poligon.get_area()
                k2 += 1

            elif z2 < z_i and z1 > z_i and z3 > z_i:
                poligon = pn.Polygon(np.array([p2,p21,p23]))
                area += poligon.get_area()
                k3 += 1

            elif z3 < z_i and z1 > z_i and z2 > z_i:
                poligon = pn.Polygon(np.array([p3,p23,p31]))
                area += poligon.get_area()
                k4 += 1
    ...
    # Function that checks if a normal vector is outward or inward facing with respect to the center of the polyhedron
    def orientation(self, normal, center, point):

    # normal: normal vector to be checked
    # center: coordinates of the icosahedron's center
    # point:  point of reference in the corresponding face (ideally it center point)
    ...
    # Function that creates a creates a list containing the area of each face that is under (or intersecting, in which case
    # the function outputs the corresponding polygon's area) a certain height Z, and relates each of those areas to the normal 
    # vector that characterizes said face
    def area_normal(self, z_i, a):


        # Defino una lista que contendrá los pares (area, vector)
        areas = []
        normals = []
        # Defino el área de una cara (triángulo equilatero de lado a)
        area_cara = a*a*np.sqrt(3)/4

        # Defino el centro del icosaedro
        center = self.center()
        ...
        for face in self.faces: 
            ...
            # Si todos los puntos están por encima de z_i se pasa a la siguiente cara
            if z1 > z_i and z2 > z_i and z3 > z_i:
                continue

            # Si todos los puntos están por debajo de z_i, el area es la del triangulo completo
            elif z1 < z_i and z2 < z_i and z3 < z_i:
                area = area_cara
                

            # En el resto de casos se calcula el polígono que se encuentra bajo z_i, y se obtiene su área
            elif z1 < z_i and z2 > z_i and z3 > z_i:
                poligon = pn.Polygon(np.array([p1,p21,p31]))
            ...
            # Teniendo ya el área, puedo proceder a calcular la normal a la cara
            
            r1 = np.array(p2) - np.array(p1)
            r2 = np.array(p3) - np.array(p1)
            cross = np.cross(r1,r2)
            cross = cross/np.linalg.norm(cross)
            punto = (np.array(p1) + np.array(p2) + np.array(p3))/3            

            normal = self.orientation(cross, center, punto)

            # Una vez calculado el vector normal, actualizo out y prosigo con el bucle
            areas.append(area)
